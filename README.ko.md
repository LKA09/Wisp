...
4. **GitHub Release에 모든 에셋이 올라왔는지 확인한 뒤 다음 단계로 넘어간다:**
...
### 중요: 배포 순서를 반드시 지켜야 한다

npm postinstall 스크립트는 의도적으로 non-fatal로 설계되어 있다 — 바이너리 다운로드에 실패하면 소스 빌드 안내를 출력하고 정상 종료된다. 즉 GitHub Release 에셋이 없어도 `npm publish`는 성공한다. **반드시 GitHub Release 에셋이 모두 확인된 이후에 npm에 배포해야 한다.** 순서가 바뀌면 사용자가 패키지를 설치하더라도 런타임에 바이너리를 찾지 못하는 오류가 발생할 수 있다.
