...
> 이 섹션은 GitHub와 npm에 Wisp 릴리스를 배포하는 메인테이너를 위한 내용이다.

### 릴리스 순서

1. **`wisp/Cargo.toml`의 버전만 올리면 된다:**
   - `wisp/Cargo.toml` — `version = "x.y.z"`
   - `npm/package.json` — CD 워크플로우가 자동으로 업데이트
...
3. **버전 태그를 만들고 푸시한다** (또는 같은 태그로 Release 워크플로우를 수동 실행한다):
...
4. **GitHub Release에 모든 에셋이 올라왔는지 확인한다** (이후 npm 배포는 CD 워크플로우가 자동으로 처리):
...
CD 워크플로우가 GitHub Release 완료 후 자동으로 npm에 배포하므로 수동 배포는 필요 없다.

### 중요: 배포 순서

npm postinstall 스크립트는 의도적으로 non-fatal로 설계되어 있다. CD 워크플로우는 GitHub Release 잡이 완료된 이후에만 `npm publish`를 실행하므로 에셋이 항상 먼저 올라가 있음이 보장된다.

### Windows ARM64 주의사항

`wisp-windows-aarch64.exe` 에셋은 GitHub 호스팅 Windows ARM64 러너가 있어야 빌드된다. 릴리스 시점에 해당 러너를 사용할 수 없다면 Windows ARM64 에셋을 릴리스에서 제외하고, 지원 플랫폼 표에서 해당 항목을 삭제하거나 소스 빌드 전용으로 표시한다.
...
- 코드는 `wisp/src/`에 있다
- 무거운 의존성 없음 — `clap`, `serde`, `toml`, `anyhow`, `chrono`, `ureq`, `serde_json`만 사용
- PR은 `develop-ai` 브랜치로
